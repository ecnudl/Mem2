...
# 7B LoRA Training Script - Balanced Configuration for 4×48GB GPUs
# Optimized for 70-80% data coverage with GRPO n=8
# Expected memory usage: ~30-35GB per GPU
...
# Ray temporary directory - use short path to avoid Unix socket 107-byte limit
# Total socket path must be < 107 bytes, so use very short base path
export RAY_TMPDIR=/tmp/ray_m4
...
NNODES=${NNODES:-1}
NGPUS_PER_NODE=${NGPUS_PER_NODE:-4}  # Using 4 GPUs
...
EXP=lora_4gpu_balanced_20k_n8
PROJ_DIR=${PROJ_ROOT}/${EXP}

# ============================================================================
# BALANCED CONFIGURATION - Key Parameters
# ============================================================================

# Context length: Cover 70-80% of data (mean=26k tokens)
MAXLEN=20000
MAX_NEW_TOKEN=1024

# Memory agent configuration
CHUNK_SIZE=2500
MAX_CHUNKS=8  # 8 * 2500 = 20000

# GRPO sampling (closer to paper's n=16)
ROLLOUT_N=3
ROLLOUT_VAL_N=2

# Batch configuration
# Effective batch size = 12 * 8 = 96 samples per optimization step
TRAIN_BATCH_SIZE=12
PPO_MINI_BATCH_SIZE=12

# Token limit per GPU - reduced for longer sequences
MAX_TOKEN_PER_GPU=12000

# Learning rates (based on Qwen2.5 recommendations + LoRA)
ACTOR_LR=3e-5
CRITIC_LR=8e-5
LR_WARMUP_STEPS=100

# vLLM inference configuration
VLLM_GPU_MEMORY_UTIL=0.40
VLLM_MAX_BATCHED_TOKENS=8192
VLLM_MAX_MODEL_LEN=3584  # Must be >= CHUNK_SIZE + overhead (2500 + ~1000)
TENSOR_PARALLEL_SIZE=2  # Use 2 GPUs for vLLM inference
...
echo "================================================================================"
echo "MemAgent PPO Training - Balanced Configuration (4×48GB GPUs)"
echo "================================================================================"
echo "Experiment: ${EXP}"
echo "GPUs: ${CUDA_VISIBLE_DEVICES} (${NGPUS_PER_NODE} GPUs × 48GB)"
echo ""
echo "Configuration Summary:"
echo "  Context Coverage: 70-80% of training data"
echo "  MAXLEN: ${MAXLEN} tokens"
echo "  Chunk Config: ${MAX_CHUNKS} chunks × ${CHUNK_SIZE} tokens"
echo "  GRPO Sampling: n=${ROLLOUT_N} (train), n=${ROLLOUT_VAL_N} (val)"
...
echo "  LR Warmup: ${LR_WARMUP_STEPS} steps (cosine schedule)"
echo "  MAX_TOKEN_PER_GPU: ${MAX_TOKEN_PER_GPU}"
echo ""
echo "LoRA Configuration:"
echo "  Rank: 64, Alpha: 32, Target: all-linear"
echo "  Trainable Params: ~0.09B (~1.2% of base model)"
echo ""
echo "Expected Memory Usage: ~30-35GB per GPU"
echo "Expected Training Speed: Medium (balanced)"
echo "================================================================================"
echo ""
...
echo "Checkpoints saved every 300 steps"
echo "Validation runs every 15 steps"
echo ""
echo "Next steps:"
echo "  1. Check training logs in ${PROJ_DIR}"
echo "  2. Monitor memory usage: watch -n 1 nvidia-smi"
echo "  3. If OOM occurs, reduce MAX_TOKEN_PER_GPU or TRAIN_BATCH_SIZE"
echo "  4. If memory usage is low (<30GB), consider increasing MAXLEN to 25000"
echo "================================================================================"
