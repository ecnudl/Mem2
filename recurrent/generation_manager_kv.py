...
class LLMGenerationManager:
    ...
    def run_llm_loop(self, gen_batch, timing_raw) -> Tuple[DataProto, torch.BoolTensor, torch.LongTensor]:
        ...
        """
        active_num_list = [] # trace the active number of sample in each decode turn
        gen_output_list = [] # store decode outputs, used for policy optimization
        ...
        while not self.agent.done():
            with _timer('mt_prepare', timing_raw):
                ...
                combined_meta = {**meta_info, **meta_info_gen}
                stage = combined_meta.get("stage", "decode")

                if stage == "prefill" and getattr(self.agent, "pending_tensors", None):
                    tensors = self.agent.pending_tensors
                    input_ids = tensors["input_ids"]
                    attention_masks = tensors["attention_mask"]
                    position_ids = tensors["position_ids"]
                else:
                    input_ids, attention_mask_bool = pad_tensor_list_to_length(
                        messages,
                        pad_token_id=pad_token_id,
                        max_length=combined_meta["input_pad_to"],
                        left_pad=True,
                        return_mask=True,
                    )
                ...
                if stage == "prefill":
                    ...
                    if fixed:
                        position_ids = create_position_ids(attention_masks)
                logger.info(f'padding done ({stage=})')
            with _timer('mt_gen', timing_raw):
                gen_output = self.generate_with_graceful_padding(input_ids, attention_masks, position_ids, combined_meta)
                logger.info(f'generation done ({stage=})')
            with _timer('mt_update', timing_raw):
                gen_output = self.agent.update(gen_output)
                if stage != "prefill":
                    gen_output_list.append(gen_output)
                logger.info(f'agent update done ({stage=})')
        ...
        assert len(sample_index) == len(gen_batch)
        assert sum(final_mask) == len(gen_batch)
        logger.info(f"ACTIVE_TRAJ_NUM(DECODE): {active_num_list}")
