...
# 7B 多卡训练脚本（优化为3卡FSDP配置，固定使用 GPU 5、6、7；如需更改请直接修改本文件）
...
# 固定使用 3 GPU（Text Memory mode 3卡 FSDP，优化显存分配）；如需改卡，直接修改这里
...
# 3卡FSDP优化：每卡显存更充裕，可以增大token限制
# 原配置：2656 tokens/GPU (2卡)
# 新配置：4000 tokens/GPU (3卡) - 支持 chunk_size=1536 (48% 覆盖率)
MAX_TOKEN_PER_GPU=4000
