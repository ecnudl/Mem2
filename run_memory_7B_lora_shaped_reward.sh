...
# 7B LoRA Training with SHAPED REWARD - Solution for sparse reward problem
# This script uses intermediate reward signals to help the model learn faster
...
# Use 2 GPUs for LoRA training (GPUs 6,7)
...
# Memory optimization - CRITICAL for vLLM v1
unset PYTORCH_CUDA_ALLOC_CONF 2>/dev/null || true
export PYTORCH_CUDA_ALLOC_CONF=""

# ENABLE SHAPED REWARD
...
EXP=lora_1k_2gpu_r32_shaped_lr1e5
...
# CRITICAL FIX 1: Lower learning rate to avoid policy collapse
LEARNING_RATE=1e-5  # Down from 5e-5 (10x reduction)

# CRITICAL FIX 2: Optimized token limit for 2x 48GB GPUs
...
echo "========================================"
echo "LoRA PPO Training with SHAPED REWARD"
echo "========================================"
echo "Experiment: ${EXP}"
echo "GPUs: ${CUDA_VISIBLE_DEVICES} (${NGPUS_PER_NODE} GPUs)"
echo "Learning Rate: ${LEARNING_RATE} (reduced from 5e-5)"
echo "LoRA Rank: 32 (reduced from 64 for stability)"
echo "LoRA Alpha: 16"
echo "LoRA Dropout: 0.05 (added for regularization)"
echo "Shaped Reward: ENABLED (format+attempt+correct)"
echo "========================================"
