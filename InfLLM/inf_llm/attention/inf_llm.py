...
def inf_llm_forward(
    ...
    faiss=False,
    perhead=False,
    selection_head: Optional[Any] = None,  # 新增：RL 选择头
    use_rl_selection: bool = False,  # 新增：是否使用 RL
    *args, **kwargs
):
    ...
    def forward(self, query : torch.Tensor,
        ...
        if past_key_value is None:
            past_key_value = ContextManager(
                ...
                faiss,
                perhead,
                selection_head=selection_head,  # 传递 RL 选择头
                use_rl_selection=use_rl_selection,  # 传递 RL 选择标志
            )
