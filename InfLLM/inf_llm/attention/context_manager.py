...
class ContextManager:
    def __init__(self, 
                 ...
                 faiss: bool = False,
                 perhead: bool = False,
                 selection_head = None,  # RL 选择头（可选）
                 use_rl_selection: bool = False  # 是否使用 RL 选择
    ):
        ...
        self.use_rl_selection = use_rl_selection and (selection_head is not None)
    ...
    def get_block_k(self, k, score):
        ...
        score_original_shape = score.shape
        
        k = self.from_group_kv(k)
        
        # 验证形状匹配
        if k.shape[:-1] != score.shape:
            import warnings
            warnings.warn(
                f"get_block_k: Shape mismatch after from_group_kv. "
                f"k.shape={k.shape}, k_original_shape={k_original_shape}, "
                f"score.shape={score.shape}, score_original_shape={score_original_shape}"
            )
            # 尝试修复：如果维度不匹配，可能需要调整
            # 这里我们假设 k.shape[:-1] 应该等于 score.shape
            # 如果 k 的 -2 维度与 score 的最后一个维度不匹配，这是一个严重问题
            if k.shape[-2] != score.shape[-1]:
                raise ValueError(
                    f"get_block_k: Dimension mismatch. "
                    f"k.shape[-2]={k.shape[-2]} != score.shape[-1]={score.shape[-1]}, "
                    f"k.shape={k.shape}, score.shape={score.shape}"
                )
        
        # 验证 k 的形状：确保 -2 维度的大小合理
        # 注意：切片后的 k 可能小于 self.block_size，所以不能直接 assert
        actual_block_size = k.shape[-2]
        if actual_block_size == 0:
            # 如果 block_size 为 0，返回空张量
            return torch.empty(
        ...
        # 获取实际的维度大小
        actual_k_dim_size = k.shape[-2]  # k 在 -2 维度上的大小（可能是 block_size 或更小）
        actual_score_dim_size = score.shape[-1]  # score 在最后一个维度上的大小
        
        # 确保 actual_k_dim_size 和 actual_score_dim_size 匹配
        if actual_k_dim_size != actual_score_dim_size:
            import warnings
            warnings.warn(
                f"get_block_k: Dimension size mismatch. "
                f"actual_k_dim_size={actual_k_dim_size}, actual_score_dim_size={actual_score_dim_size}, "
                f"k.shape={k.shape}, score.shape={score.shape}"
            )
            # 使用较小的值作为实际大小
            actual_k_dim_size = min(actual_k_dim_size, actual_score_dim_size)
            # 如果 score 更大，截断 score
            if actual_score_dim_size > actual_k_dim_size:
                score = score[..., :actual_k_dim_size]
            # 如果 k 更大，截断 k（这种情况不应该发生，但为了安全）
            if k.shape[-2] > actual_k_dim_size:
                k = k[..., :actual_k_dim_size, :]
        
        # 确保 topk 不超过实际可用的元素数量
        actual_repr_topk = min(self.repr_topk, actual_score_dim_size)
        if actual_repr_topk <= 0 or actual_k_dim_size <= 0:
            # 如果没有可用的元素，返回空张量
            return torch.empty(
                (self.num_units, self.unit_size, self.repr_topk, self.dim_head),
                dtype=k.dtype,
        ...
        # 执行 topk，但限制在可用范围内
        score_topk = score.topk(actual_repr_topk, dim=-1).indices
        assert score_topk.shape == (self.num_units, self.unit_size, actual_repr_topk)
        
        # 关键修复：确保索引在 [0, actual_k_dim_size) 范围内
        # score.topk 返回的索引是基于 score 的最后一个维度，但我们需要确保它不超过 k 的有效范围
        # 使用 clamp 确保索引在有效范围内，并且使用 max(0, ...) 防止负数
        score_topk = torch.clamp(score_topk, 0, max(0, actual_k_dim_size - 1))
        
        # 双重验证：再次检查并修复任何无效索引
        # 使用 torch.where 确保所有索引都在有效范围内
        score_topk = torch.where(
            score_topk < 0,
            torch.zeros_like(score_topk),
            score_topk
        )
        score_topk = torch.where(
            score_topk >= actual_k_dim_size,
            torch.full_like(score_topk, max(0, actual_k_dim_size - 1)),
            score_topk
        )
        
        # 最终验证：如果仍然有无效索引，记录警告
        import warnings
        if not ((score_topk >= 0).all() and (score_topk < actual_k_dim_size).all()):
            warnings.warn(
                f"get_block_k: Invalid indices detected after clamp. "
                f"score_topk range [{score_topk.min().item()}, {score_topk.max().item()}], "
                f"actual_k_dim_size={actual_k_dim_size}, actual_score_dim_size={actual_score_dim_size}, "
                f"k.shape={k.shape}, score.shape={score.shape}. "
                f"Force clamping to [0, {actual_k_dim_size - 1}]"
            )
            # 强制修复
            score_topk = torch.clamp(score_topk, 0, max(0, actual_k_dim_size - 1))
        
        # 如果实际 topk 小于期望的 repr_topk，需要 padding
        if actual_repr_topk < self.repr_topk:
            # Padding score_topk 到期望的大小（使用最后一个有效索引，但确保不超过范围）
            last_valid_idx = torch.clamp(score_topk[:, :, -1:], 0, max(0, actual_k_dim_size - 1))
            padding = last_valid_idx.expand(self.num_units, self.unit_size, self.repr_topk - actual_repr_topk)
            score_topk = torch.cat([score_topk, padding], dim=-1)
        
        # 最终检查：确保所有索引都在有效范围内
        score_topk = torch.clamp(score_topk, 0, max(0, actual_k_dim_size - 1))
        
        # 准备 gather 索引：需要匹配 k 的形状
        # k 的形状: (num_units, unit_size, block_size, dim_head)
        # score_topk 的形状: (num_units, unit_size, repr_topk)
        # gather 操作要求索引形状与输入在非 gather 维度上匹配
        # 对于 gather(k, -2, indices)，indices 的形状应该是 (..., repr_topk, dim_head)
        # 但索引值必须在 [0, block_size) 范围内
        
        # 最后一次确保索引在有效范围内（使用更严格的方法）
        score_topk = torch.clamp(score_topk, min=0, max=max(0, actual_k_dim_size - 1))
        
        # 验证：确保所有索引都在有效范围内
        if actual_k_dim_size > 0:
            # 使用 torch.where 确保索引严格在范围内
            score_topk = torch.where(
                score_topk < 0,
                torch.zeros_like(score_topk),
                score_topk
            ...
            score_topk = torch.where(
                score_topk >= actual_k_dim_size,
                torch.full_like(score_topk, actual_k_dim_size - 1),
                score_topk
            )
        else:
            # 如果 actual_k_dim_size == 0，所有索引都应该是 0
            score_topk = torch.zeros_like(score_topk)
        
        # 扩展索引以匹配 k 的形状
        # k 的形状: (num_units, unit_size, block_size, dim_head)
        # score_topk 的形状: (num_units, unit_size, repr_topk)
        # 对于 gather(k, -2, indices)，indices 的形状必须与 k 在非 -2 维度上匹配
        # 即 indices 的形状应该是 (num_units, unit_size, repr_topk, dim_head)
        # 但索引值只在 -2 维度（block_size）上使用
        
        # 获取实际的 dim_head 大小（从 k 的形状）
        actual_dim_head = k.shape[-1]
        
        # 扩展索引：在最后一个维度上扩展以匹配 k 的形状
        # 使用 unsqueeze 和 expand 来扩展维度
        gather_indices = score_topk.unsqueeze(-1)  # (num_units, unit_size, repr_topk, 1)
        gather_indices = gather_indices.expand(self.num_units, self.unit_size, self.repr_topk, actual_dim_head)
        
        # 最终验证：确保所有索引都在有效范围内
        if actual_k_dim_size > 0:
            # 确保索引是 long 类型（gather 要求）
            gather_indices = gather_indices.long()
            
            # 使用同步操作确保所有检查完成（如果使用 CUDA）
            if k.is_cuda:
                torch.cuda.synchronize()
            
            # 再次 clamp 确保安全（在转换为 long 之后再次检查）
            gather_indices = torch.clamp(gather_indices, min=0, max=max(0, actual_k_dim_size - 1))
            
            # 最终检查：如果仍有问题，使用更保守的方法
            invalid_mask = (gather_indices < 0) | (gather_indices >= actual_k_dim_size)
            if invalid_mask.any():
                warnings.warn(
                    f"get_block_k: Found invalid indices after all checks. "
                    f"gather_indices range [{gather_indices.min().item()}, {gather_indices.max().item()}], "
                    f"actual_k_dim_size={actual_k_dim_size}, k.shape={k.shape}, "
                    f"score_topk.shape={score_topk.shape}, invalid_count={invalid_mask.sum().item()}"
                )
                # 强制将所有无效索引设为 0
                gather_indices = torch.where(invalid_mask, torch.zeros_like(gather_indices), gather_indices)
                # 再次 clamp
                gather_indices = torch.clamp(gather_indices, min=0, max=max(0, actual_k_dim_size - 1))
        else:
            # 如果 actual_k_dim_size == 0，创建零索引
            gather_indices = torch.zeros(
                (self.num_units, self.unit_size, self.repr_topk, actual_dim_head),
                dtype=torch.long,
                device=k.device
            )
        
        # 确保索引是 long 类型（gather 要求）
        if gather_indices.dtype != torch.long:
            gather_indices = gather_indices.long()
        
        # 执行 gather 操作
        # 在 gather 之前再次验证形状匹配
        expected_shape = (self.num_units, self.unit_size, self.repr_topk, actual_dim_head)
        if gather_indices.shape != expected_shape:
            raise ValueError(
                f"get_block_k: gather_indices shape mismatch. "
                f"Expected {expected_shape}, got {gather_indices.shape}"
            )
        
        ret = torch.gather(k, -2, gather_indices)
        return ret
    ...
    def calc_block_topk(
        self, global_h_q,
        selection_head=None,  # 新增参数（向后兼容）
        use_rl=False  # 新增参数（向后兼容）
    ):
        # 如果启用 RL 选择，使用 RL 方法
        if use_rl or (self.use_rl_selection and self.selection_head is not None):
            # 如果传入了 selection_head，临时使用它
            if selection_head is not None:
                original_head = self.selection_head
                original_use_rl = self.use_rl_selection
                self.selection_head = selection_head
                self.use_rl_selection = True
                try:
                    result = self.calc_block_topk_rl(global_h_q)
                finally:
                    self.selection_head = original_head
                    self.use_rl_selection = original_use_rl
                return result
            else:
                return self.calc_block_topk_rl(global_h_q)
    ...
    def calc_block_topk_rl(
        self,
        global_h_q: torch.Tensor,
    ):
        """
        使用 RL 选择头进行块选择
        
        Args:
            global_h_q: (num_units, unit_size, seq_len, dim_head) 或 (num_units, unit_size, dim_head)
        
        Returns:
            List[List[int]]: 每个 unit 选择的块索引列表
        """
        if not self.use_rl_selection or self.selection_head is None:
            # 回退到固定检索
            return self.calc_block_topk(global_h_q)
        
        # 处理输入维度
        if global_h_q.dim() == 4:
            # (num_units, unit_size, seq_len, dim_head) -> (num_units, unit_size, dim_head)
            global_h_q = global_h_q.mean(dim=2, keepdim=False)
        ...
        # 准备 query 向量: (num_units, unit_size, dim_head) -> (num_units, dim_head * unit_size)
        query_vec = global_h_q.reshape(self.num_units, self.dim_head * self.unit_size)
        hidden_size = self.dim_head * self.unit_size
        
        # 准备块表示向量
        # block_k[u].get_data() 返回 (num_blocks, dim_head * unit_size)
        block_reprs_list = []
        for u in range(self.num_units):
            block_data = self.block_k[u].get_data()  # (num_blocks, hidden_size)
            block_reprs_list.append(block_data)
        
        # 转换为张量: (num_units, num_blocks, hidden_size)
        # 需要 padding 到相同的块数量
        max_blocks = max(len(br) for br in block_reprs_list) if block_reprs_list else 0
        if max_blocks == 0:
            return [[] for _ in range(self.num_units)]
        
        # 创建 padded 张量
        block_reprs = torch.zeros(
            (self.num_units, max_blocks, hidden_size),
            dtype=query_vec.dtype,
            device=query_vec.device
        )
        
        for u in range(self.num_units):
            num_blocks_u = len(block_reprs_list[u])
            if num_blocks_u > 0:
                block_reprs[u, :num_blocks_u] = block_reprs_list[u]
        
        # 使用选择头
        # query_vec: (num_units, hidden_size)
        # block_reprs: (num_units, max_blocks, hidden_size)
        with torch.set_grad_enabled(self.selection_head.training):
            scores, probs, indices = self.selection_head(
                query_vec,  # (num_units, hidden_size)
                block_reprs,  # (num_units, max_blocks, hidden_size)
                topk=self.topk,
                training=self.selection_head.training,
            )
        
        # indices: (num_units, topk)
        # 转换为 List[List[int]]
        ret = []
        for u in range(self.num_units):
            num_blocks_u = len(block_reprs_list[u])
            if num_blocks_u == 0:
                ...
                # 过滤掉超出实际块数量的索引
                valid_indices = indices[u].cpu().tolist()
                valid_indices = [idx for idx in valid_indices if idx < num_blocks_u]
                ret.append(valid_indices)
