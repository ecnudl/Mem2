#!/bin/bash
# Quick Evaluation Script for 7B LoRA 2-GPU Training Results
...
show_help() {
    cat << EOF
Evaluate 7B LoRA 2-GPU Training Results (lora_1k_2gpu_r64)
...
Options:
    ...
    --length LENGTH         Evaluation length (50,100,200,400,800,1600,3200,6400)
                            Default: 100
    --step STEP            Evaluate single checkpoint (e.g., 500, 1000, 1250)
                            Default: evaluate all (500 1000 1250)
...
Examples:
    ...
    # Evaluate only step 1250 at length 200
    bash $(basename $0) --step 1250 --length 200

    # Only merge all checkpoints without evaluation
    bash $(basename $0) --merge-only

    # Re-evaluate step 1000 (skip merge, force new eval)
    bash $(basename $0) --step 1000 --eval-only --force-eval

Checkpoints available:
    - global_step_500
    - global_step_1000
    - global_step_1250 (final)
...
Training configuration:
    ...
    - Max New Tokens: 64
    - Training GPUs: 6,7
    - Evaluation GPU: 4 (to avoid conflict)
EOF
}
